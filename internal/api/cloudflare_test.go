...
const (
	...
	mockTokenVerifyResponse = `{"result":{"id":"token123","status":"active"},"success":true,"errors":[],"messages":[{"code":10000,"message":"This API Token is valid and active","type":null}]}`
	mockAccountResponse     = `{"result":{"id":"account567","name":"account-name"},"success":true,"errors":[],"messages":[]}`
)
...
func newHandle(t *testing.T, ppfmt pp.PP, accountID string,
	...
	mux.HandleFunc(fmt.Sprintf("GET /accounts/%s", accountID), func(w http.ResponseWriter, r *http.Request) {
...
func TestSanityCheck(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		"account-illformed/1": {
			...
			mockAccountID,
			http.StatusOK, mockTokenVerifyResponse,
			http.StatusBadRequest, `{"result":null,"success":false,"errors":[{"code":7003,"message":"Could not route to the black hole, perhaps your object identifier is invalid?"}],"messages":[]}`,
			true, false,
			func(p *mocks.MockPP) {
		...
		"account-illformed/2": {
			...
			mockAccountID,
			http.StatusOK, mockTokenVerifyResponse,
			http.StatusNotFound, `{"result":null,"success":false,"errors":[{"code":7003,"message":"Could not route to the white hole, perhaps your object identifier is invalid?"}],"messages":[]}`,
			true, false,
			func(p *mocks.MockPP) {
				p.EXPECT().Errorf(pp.EmojiUserError, "The Cloudflare account ID is invalid; please check the value of CF_ACCOUNT_ID") //nolint:lll
			},
